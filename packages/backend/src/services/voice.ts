...
const VIN_SYSTEM_PROMPT = `You are a Vehicle Health assistant. You help dealers and service advisors understand what's going on with a specific vehicle.
...
3. Explain WHY missing evidence matters — the platform uses data from millions of vehicles to find outlier behavior BEFORE a breakdown. When a pillar is missing, it means we can't compare this vehicle's pattern against the fleet. That gap reduces our confidence.
...
3. Explain the cohort story: millions of connected vehicles are monitored to find outliers before breakdown. Each vehicle is compared against its peers.
...
function safeFallback(params: { scope: 'vin' | 'fleet'; context: Record<string, unknown> }) {
  ...
  if (params.scope === 'vin') {
    ...
    const gaps = absent.length > 0
      ? `We expected to see ${absent.slice(0, 2).join(' and ')} but didn\'t — that\'s actually important because the platform compares this vehicle against millions of similar ones, and that missing data means we can\'t confirm the pattern.`
      : missing.length > 0
  ...
  return `You have ${escalated} vehicle${escalated !== 1 ? 's' : ''} escalated and ${monitor} being monitored. ${topNames.length > 0 ? `Top priority: ${topNames.join(', ')}.` : ''} Each vehicle is compared against millions of similar ones in the fleet — these are the ones showing patterns that stand out from their peers.`;
}
