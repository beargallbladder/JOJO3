...
export const HEALTH_STORYLINES: HealthStoryline[] = [
  ...
  {
    id: 11,
    name: 'Acute Cardiovascular Crisis',
    description: 'High concern with strong evidence and fresh data. HRV tanked, HR spiking, iron panel critical. Should trigger ESCALATED governance.',
    primaryDomain: 'cardiovascular',
    pCurve: (t) => clamp(0.75 + t * 0.2),
    cCurve: (t) => clamp(0.65 + t * 0.25),
    sCurve: (t) => clamp(0.05 + t * 0.05),
    activeSignals: ['resting_hr', 'hrv_rmssd', 'lab_hemoglobin', 'lab_ferritin', 'lab_iron', 'strain_score'],
  },
  {
    id: 12,
    name: 'Metabolic Emergency',
    description: 'A1c and glucose climbing fast. CRP elevated. Strong evidence, high concern. ESCALATED.',
    primaryDomain: 'metabolic',
    pCurve: (t) => clamp(0.80 + t * 0.15),
    cCurve: (t) => clamp(0.70 + t * 0.20),
    sCurve: (t) => clamp(0.05 + t * 0.03),
    activeSignals: ['lab_a1c', 'lab_fasting_glucose', 'lab_crp', 'active_calories', 'nutrition_calories'],
  },
  {
    id: 13,
    name: 'MSK Breakdown',
    description: 'Pain spiking, CRP high, CK elevated, recovery tanked. Needs immediate deload. ESCALATED.',
    primaryDomain: 'musculoskeletal',
    pCurve: (t) => clamp(0.78 + t * 0.18),
    cCurve: (t) => clamp(0.68 + t * 0.22),
    sCurve: (t) => clamp(0.04 + t * 0.04),
    activeSignals: ['self_pain', 'lab_crp', 'lab_creatine_kinase', 'recovery_score', 'strain_score'],
  },
];
